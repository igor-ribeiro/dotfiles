...
const duplicatedFilename = join(chunksFolder, `DUPLICATED-${rawFilename}.${ext}`);
...
function saveChunk() {
  ...
  const chunkFileName = `${rawFilename}-${String(chunksCount).padStart(2, 0)}.${ext}`;
...
rl.on('line', line => {
  ...
  console.log('-'.repeat(20))
  console.log(`- Generated ${chunksCount} file(s) with ${linesPerFile} lines each on ${chunksFolder}`);
  console.log(`- Ignored ${ignoredCount} duplicated line(s) saved on ${duplicatedFilename}`);
